...
### 2. Compute Skill Vectors

For each position $s_i$ in the dataset, we compute the **skill vector**:
...
### 3. Cluster Skill Vectors

We apply the clustering algorithm of choice (like **K‑Means**, **DBSCAN**, or **Density Peak Clustering**) to the normalised skill vectors $\hat \delta_i$ to produce $B$ clusters. Each cluster defines a **bucket** of positions that share a similar learning signal.

### 4. Train a Dispatcher

Since **skill vectors cannot be computed at inference time** (they require gradients and correct labels for $\hat v$), we train a **dispatcher** $g_\phi(h)$ to predict the bucket ID from the **L1 activations** $h$.
...
| ---------------------- | --------------------- | -------------------- | ------- | -------- | ----------- |
| **Linear** (no hidden) | `linear_wdl_smoke`    | 844×2 → 3            | 5 067   | 0.7760   | **0.7645**  |
| **1 hidden** (medium)  | `medium_h20`          | 844×2 → 20 CReLU → 3 | 33 843  | 0.7149   | **0.6872**  |
| **2 hidden** (NNUE)    | `dual_h128_H256_fast` | L1 128×2, L2 256 → 3 | 174 723 | 0.6797   | **0.6966**  |
...
### MoE with Skill Vector Bucketing (our approach)
